...
/*   Updated: 2024/11/03 17:12:22 by eschussl         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int orient(Point const a, Point const b, Point const c) // remember that c is the point we are trying to find
{
	...
	Point ac(a, c);
	float crossProduct = ab.getX().toFloat() * ac.getY().toFloat() - ac.getX().toFloat() * ab.getY().toFloat(); // to make a cross product i to draw a vector that is perpendicular to 2 other vectors
	if (roundf(crossProduct) >= 0) // if cross product is < 0 then ab is left of ac so to speak
		return (1);
	return (-1);
}
